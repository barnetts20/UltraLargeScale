...
// Tier generation for the galaxy layer. NO FIELD EVALUATION HERE: the
// density field lives in GalaxyDensityCore.ush and is evaluated only by the GPU, so the stars
// and the gas are one function rather than two kept in agreement. This module marshals and
// reads back.
...
void GalaxyDataGenerator::Initialize()
{
	...
	// calls back through here on an object that already ran a different galaxy. A SURVIVING
	// CACHE ENTRY IS A SILENT WRONG ANSWER, and the only trace is the ABSENCE of the
	// "calibrated tier" log line.
	...
	bool bInCellsShareSlot) const
{
	...
	// ONCE PER TIER, OVER ITS WHOLE GRID. Never per batch. Accepted count per cell is
	// BudgetScale x mass_i, so solving it from a BATCH makes a cell's yield depend on which
	// neighbours streamed in beside it: empty sky is forced to the same total as a
	// neighbourhood full of arms, so voids come back populated.
	FScopeLock Lock(TierBudgetScaleLock.Get());
	...
	// THE PARAMETERS THIS TIER IS ACTUALLY RUNNING WITH. A parameter that fails to arrive looks
	// exactly like one that has no effect, and a generator holds a COPY taken at
	// InitializeData, so an edit can land on a struct nothing reads. THE SUBDIVISION IS LOGGED
	// CLAMPED, not authored, because the clamp is silent.
	UE_LOG(LogTemp, Log,
	...
	// MEASURE THE CELLS GENERATION WILL ACTUALLY USE. Both tier kinds descend to the generation
	// grid, because a cell's mass is the mean over its own volume: probing an undivided parent
	// whose structure occupies a few percent of it reports a mean too low, as BIAS growing with
	// the subdivision depth, so the constant comes out too high and generation over-delivers.
	...
	if (AllCells.Num() > 0 && Params.Procedural.GetFieldTextures().IsComplete())
	{
			...
			&& CellMass.Num() == AllCells.Num())
		{
			...
			// cells produce, so the divisor is whichever sum the slot actually sees:
			//
			//   cells SHARE a slot  -> the TOTAL over the grid, the large tier feeding its whole
			//     subdivided grid into one slot.
			//   one cell PER slot   -> the LARGEST PER-PARENT SUM, a streamed cell owning a slot
			//     its children all write to. A total would force every neighbourhood to the same
			//     count -- which is how a void came back as full as an arm.
			//
			// Fixed array order so the answer is reproducible, and double because a subdivided
			// grid is hundreds of thousands of terms.
			double Total = 0.0;
			...
			// Taken even when the tier does not divide by it: the two side by side in the log
			// say whether a tier is calibrated against the right one -- a share-slot tier's
			// total should dwarf its largest parent, a per-slot tier's should not.
			...
			// THE PER-GALAXY DENSITY KNOB, APPLIED HERE AND NOWHERE ELSE. It scales the TARGET
			// rather than the field, the only place it can go: scaling the field does nothing,
			// since acceptance normalises against each cell's own probed envelope. Scaling
			// SlotCapacity would resize the Niagara buffers, allocated once per pooled actor.
			// Solving BudgetScale against a reduced capacity makes a sparse galaxy GENERATE
			// fewer entities rather than being thinned afterwards.
			//
			...
			// excess is dropped by the cursor backstop, silently and in arrival order.
	...
	// Cached even at zero: a retry every batch would re-probe an entire grid on the streaming
	// path, and no texture or no device does not resolve itself between boundary crosses.
	...
	TArray<FTierBatchCell>& OutCells) const
{
	...
	// The streaming system's grid, enumerated exhaustively rather than around a viewer. Same
	// formula, because calibration has to measure the cells the tier will generate with -- a
	// different coord labelling would reseed the probe jitter and give a different answer.
	...
	// THE SAME CULL THE SUBDIVISION USES, and it has to be: two spellings of "outside the unit
	// sphere" would let calibration weigh a set of cells generation never produces.
	...
	for (int32 iz = -Ring; iz <= Ring; ++iz)
	{
		for (int32 iy = -Ring; iy <= Ring; ++iy)
		{
			for (int32 ix = -Ring; ix <= Ring; ++ix)
			{
				...
				// Beyond the unit sphere the field is zero everywhere in the cell.
...
{
	// GENERATION GRANULARITY, SEPARATED FROM SLOT GRANULARITY. A streamed cell is sized so a
	// neighbourhood stays resident; the field's structure has no reason to match. With a disc a
	// few percent of a cell thick the mean density is a small fraction of the peak, and
	// rejection against a per-cell envelope accepts at exactly that ratio -- which is why
	// acceptance sat around one percent. Descending inside each cell culls children clear of the
	// structure and leaves survivors with a peak closer to their own mean, keeping the PARENT'S
	// SLOT throughout.
	...
	// FAIL CLOSED, VISIBLY. There is no CPU path behind this, and a slot is REUSED as the
	// player crosses boundaries, so "nothing written" is the previous occupant's entities
	// sitting at a coord they do not belong to -- which reads as a placement bug rather than a
	// generation failure. Blanking costs an empty region, which is honest.
	...
	// misconfiguration would otherwise surface only as a tier that never populates, reading as
	// a streaming problem rather than a setup one.
	if (!Params.Procedural.GetFieldTextures().IsComplete())
	{
		...
		// check four bags and four defaults when three are usually fine.
		ensureMsgf(false,
	...
	// One cell per queued slot. The KEY is the grid coord, never the dispatch index: identity
	// is (coord, slot), which is what makes a region regenerate identically on revisit.
	TArray<FGalaxyGenCell> Cells;
	Cells.Reserve(InCells.Num());

	// ONE SHARED BUFFER, sized on what the SLOTS can hold, not on cells. No cell has a ceiling,
	// so the only question is how many entities can possibly be KEPT. Sizing per cell means a
	// uniform run as wide as the densest cell needs with every cell paying for it: either
	// hundreds of megabytes of readback, or a run dense cells pin against, a cubic lattice.
	...
	// The tier's calibrated constant, measured once over its whole grid and never from this
	// batch. Whether cells share slots is STATED by the caller, not counted here: subdivision
	// also makes cells outnumber slots and means the opposite thing, so the constant still
	// comes from the largest single streamed cell.
	...
	// AN EIGHTH OVER TARGET, against stochastic overshoot alone. Calibration measures the cells
	// generation will use, so the realised count lands ON the slot and what is left is its
	// square root -- about 0.7% at a slot of twenty thousand. Headroom exists because the
	// alternative when it runs out is the GPU truncating by arrival order, biased spatially.
	...
	for (const FTierBatchCell& In : InCells)
	{
		// GEOMETRY ONLY: what this cell holds, rejects against and draws is its group's
		// business.
		FGalaxyGenCell Cell;
		Cell.Centre = FVector3f(
			static_cast<float>(In.Centre.X),
		...
		// The KEY is the grid coord, never the dispatch index -- identity is (coord, slot).
		Cell.Coord = FIntVector3(In.Coord.X, In.Coord.Y, In.Coord.Z);

		Cells.Add(Cell);
	}

	// SIZE GUARD. Both axes are bounded by construction, so this can only fire on an absurd
	// authored capacity, never on the cell count.
	...
	// Confirms the compute path ran AND produced something: an uncleared counter, an empty batch
	// or a saturated bounds test all return success and place nothing, and one tier populated
	// with the others empty reads as a streaming problem. Counts is FIVE per cell, then cursor:
	//   [0] accepted   [1] candidates evaluated   [2] the cell's envelope, as asuint
	...
	// THE TARGET THIS BATCH WAS SOLVED FOR, summed from the same counter calibration reduced:
	// accepted_i is BudgetScale x mass_i, so BudgetScale times the batch's own mass is what it
	// should land on, which works for a partial neighbourhood too.
	...
	for (int32 i = 0; i < InCells.Num(); ++i)
	{
		...
		if (Envelope <= 0.0f)
		{
			// Culled by the shader's own probes -- most of a galaxy's bounding grid is void.
			continue;
		}

		++LiveCells;

		// THE ENVELOPE CHECK: a candidate found more density than the cell's whole probe set
		// did, so the ratio clipped at 1 and the peak was flattened. What EnvelopePad exists to
		// prevent, with no other observer.
	...
	// Once per run while things are healthy, and every time they are not. The GLOBAL cursor,
	// one past the per-cell region, deliberately un-clamped by the shader so it reports the
	// true total accepted even on overflow -- exactly the case the CPU needs to know about.
	const uint32 GlobalAccepted =
		Counts[GalaxyEntityGen::GlobalCursorIndex(InCells.Num())];

	// REDUCED IN uint32 BEFORE IT NARROWS. The counter is un-clamped so it reports the true
	// total even on overflow, the one case where a value past INT32_MAX is reachable -- and
	// narrowing first makes it NEGATIVE, so the overflow test reports no overflow and the
	// warning never fires, on the exact input the counter exists to report.
	...
	// ATOMIC, because this runs on background workers for several galaxies at once: a plain
	// bool is a data race, benign in effect but one the sanitizers are right to flag.
	...
	if (bFirst || TotalAccepted == 0 || bOverflowed || ExceededCells > 0)
	{
		...
		// PROBES AND CANDIDATES ARE BOTH FIELD EVALUATIONS, so their ratio says whether the tier's
		// GenerationSubdivision sits on the right side of its crossover: below about 1 the tier
		// wants one level fewer, above about 9 it wants one more.
	...
	// THE IDENTITY THIS TIER RESTS ON, CHECKED RATHER THAN ASSUMED. A cell draws BudgetScale x
	// (envelope/anchor)^g candidates and keeps each with probability p, and the envelope divides
	// out, so the batch should land on the number calibration solved BudgetScale for -- and
	// until this, a batch delivering half its target looked exactly like one delivering all.
	//
	// NOT THE SAME QUESTION AS THE COUNTERS BESIDE IT: envelope-exceeded says the probes
	// under-weighed a CELL, this says the tier as a whole is off. Wide band, because the
	// realised count varies by roughly its square root: it looks for a factor, not a percent.
	...
	// deterministically -- everything within the headroom is thinned uniformly below, this is
	// the overshoot beyond it. It should not be reachable: the buffer holds twice the target and
	// the shader solves the budget to land ON it.
	if (bOverflowed)
	{
	...
	// Scatter, with a WRITE CURSOR PER SLOT. Entities arrive in one shared, globally-appended
	// buffer rather than per-cell runs, so an entity's POSITION says nothing about where it
	// belongs: each record carries its producing cell, and the cell carries the slot. Nothing
	// downstream may key off buffer order -- identity is (cell, slot).
	...
	// Every queued slot is touched whether or not anything landed in it: one that generated
	// nothing still has to be padded dead, or it keeps the last coord's entities.
	...
	// POOLED BUDGET, APPLIED AT SCATTER TIME. A slot can still receive more than it holds, and
	// WHICH entities survive is the whole question: arrival order answers it by scheduling, cell
	// order by grid position with the cut along grid planes. Thinning keeps a uniform fraction
	// of every cell's entities instead. AFTER GENERATION RATHER THAN BEFORE, since dividing a
	// pooled budget up front needs the batch total in advance.
	...
	// Five percent of headroom: the keep test is per entity and independent, so the realised
	// count varies around the target by roughly its square root.
	...
	for (int32 i = 0; i < Landed; ++i)
	{
		...
		// A BACKSTOP, not the mechanism: if this trips with thinning active the headroom was
		// too small and the discard goes back to being order-dependent.
		...
		if (Keep < 1.0f)
		{
			// Keyed on (coord, slot), never on i: append order is scheduling-dependent, so
			// thinning on the buffer index would make the stars flicker between visits.
	...
	// Padded once per slot, AFTER every cell that feeds it has been written; inside the loop it
	// would blank the entities the previous cell just placed.
