...
FastNoise::SmartNode<> UniverseDataGenerator::BuildNoise() const
{
	...
	DomainScale->SetScale(Params.DensityParams.MasterScale);
	...
	Remap0->SetRemap(0, 1, Params.DensityParams.ClusterRemapMax, Params.DensityParams.ClusterRemapMin);
	auto Pow0 = FastNoise::New<FastNoise::PowInt>();
	Pow0->SetValue(Remap0);
	Pow0->SetPow(Params.DensityParams.ClusterFalloff);
	auto Scale0 = FastNoise::New<FastNoise::DomainScale>();
	Scale0->SetSource(Pow0);
	Scale0->SetScale(Params.DensityParams.ClusterScale);
	auto Pow1 = FastNoise::New<FastNoise::PowInt>();
	Pow1->SetValue(Fbm0);
	Pow1->SetPow(Params.DensityParams.WebFalloff);
	auto Mul0 = FastNoise::New<FastNoise::Multiply>();
	Mul0->SetLHS(Scale0);
	Mul0->SetRHS(Pow1);
	auto Mul1 = FastNoise::New<FastNoise::Multiply>();
	Mul1->SetLHS(Mul0);
	Mul1->SetRHS(Params.DensityParams.ClusterMulti);
	auto Remap1 = FastNoise::New<FastNoise::Remap>();
	Remap1->SetSource(Pow1);
	Remap1->SetRemap(0, 1, Params.DensityParams.WebRemapMin, Params.DensityParams.WebRemapMax);
	auto Add0 = FastNoise::New<FastNoise::Add>();
	Add0->SetLHS(Remap1);
	Add0->SetRHS(Mul1);
	auto Warp0 = FastNoise::New<FastNoise::DomainWarpGradient>();
	Warp0->SetSource(Add0);
	Warp0->SetWarpAmplitude(Params.DensityParams.WarpAmp);
	Warp0->SetWarpFrequency(Params.DensityParams.WarpFreq);
	return Warp0;
}
