	...
	ParallelFor(Count, [this, InOctree](int32 i)
		{
			...
			InOctree->InsertPosition(FVector(InsertPosition), InsertDepth, InsertData);
		});
}
	...
	ParallelFor(Count, [this, &InsertPositions, &InsertDepths, &InsertPayloads, Extent, Noise, InOctree](int32 i)
		{
			...
			FLinearColor Color = FLinearColor::MakeRandomSeededColor(i);
			FVector Composition(Color.R, Color.G, Color.B);
			InOctree->InsertPosition(FVector(InsertPosition), Stream.RandRange(MinInsertionDepth, MaxInsertionDepth), FVoxelData(Stream.FRand(), Stream.FRand() * FMath::Pow(FMath::Max(T, 0.000001), 6), Composition, i, Type));
		}
	, EParallelForFlags::BackgroundPriority);
}
	...
	ParallelFor(Count, [this, InOctree, SeedOffset, DomainScale](int32 i)
		{
			...
			InOctree->InsertPosition(FVector(Coord), InsertDepth, Data);
		});
}
	...
	ParallelFor(Count, [this, InOctree, SeedOffset, DomainScale, Noise, DistortionNoise](int32 i)
		{
			...
			int32 InsertDepth = MaxInsertionDepth;// GalaxyDataGenerator::ChooseDepth(Stream.FRand());
